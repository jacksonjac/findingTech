...
export const GetAllChatsListbyTechid = (dependencies: any) => {
  console.log(" get alll chatlist  funtions");
  const { getChatlistbyTechidRepo } = dependencies.repositery;

  const executeFunction = async (id:any) => {
    try {
      // Fetch user data
      const responseFromUserList = await getChatlistbyTechidRepo.PostExit(id);
