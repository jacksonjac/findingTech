...
export const getOneUserById = (dependencies: any) => {
    const { getOneUserbyidRepo } = dependencies.repositery;

    const executeFunction = async (userid: any) => {
        try {
            // Fetch user data
            const responseFromUserList = await getOneUserbyidRepo.PostExit(userid);
