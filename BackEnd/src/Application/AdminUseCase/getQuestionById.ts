...
export const getOneQuestionById = (dependencies: any) => {
    const { getOneQuestionbyIdRepo } = dependencies.repositery;

    const executeFunction = async (userid: any) => {
        try {
            // Fetch user data
            const responseFromUserList = await getOneQuestionbyIdRepo.PostExit(userid);
