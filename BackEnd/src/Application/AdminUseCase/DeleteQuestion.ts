...
export const DeleteQuestion = (dependencies: any) => {
    const { DeleteQuestionRepo } = dependencies.repositery;

    const executeFunction = async (userid: any) => {
        try {
            // Fetch user data
            const responseFromUserList = await DeleteQuestionRepo.PostExit(userid);
